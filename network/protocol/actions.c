...
{
    struct response_actions_s *response = (struct response_actions_s *)buffer;
...
static void send_response(struct actions_object_s* self, int udpfd)
{
    ...
    memset(&response, 0, sizeof(struct actions_object_s));
    ...
    create_payload(to_send, &response, sizeof(struct actions_object_s));

    sendto(udpfd, to_send, sizeof(to_send), 0, 
					(struct sockaddr*)self->client, sizeof(struct sockaddr));
